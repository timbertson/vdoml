...
module Make(App:App_.S) = struct
  ...
  type vdom_attr = message Attr.optional
  type vdom_property = message Attr.property
  ...
  module Tyxml_impl = struct
    module Node : sig
      ...
      let create tag (attrs : message Attr.optional list) children =
        Anonymous (Element {
          e_name = tag;
      ...
      let svg tag (attrs : message Attr.optional list) children =
        Anonymous (Element {
          e_name = tag;
          e_attrs = Attr.list_to_attrs attrs;
          e_children = children
    ...
    module Xml = struct
      ...
      type mouse_event_handler = Dom_html.mouseEvent Js.t -> event_response
      type keyboard_event_handler = Dom_html.keyboardEvent Js.t -> event_response
      type attrib = message Attr.optional
  ...
  let a_on event (handler : (Dom_html.event) Js.t -> event_response) : message Attr.optional =
  ...
  let raw_handler handler = Attr.Event_handler (handler:>(biggest_event Js.t -> event_response))
  let emit msg = Attr.Message msg
  let emit_f fn = Attr.Message_fn (fn:>(biggest_event Js.t -> message option))
